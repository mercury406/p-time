...
@section('content')
    <form action="{{ route('admin.shahars.store') }}" method="POST">
        @csrf
        ...
        <div class="mb-3 input-group">
            <select name="region_id" class="form-control">
                <option value="-1" selected disabled>Выберите область</option>
            @foreach ($regions as $region)
                <option value="{{$region->id}}">{{ $region->translate('title', 'ru') }}</option>
            @endforeach
            </select>
        </div>
    ...
    <script>
        document.getElementById('check_slug_btn')
                .addEventListener('click', () => {
                    ...
                    fetch("{{ route('admin.shahars.check-slug') }}" + `?slug=${slug}`, )
